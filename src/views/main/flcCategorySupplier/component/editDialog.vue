				...
				<el-row :gutter="35">
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="24" class="mb20">
						<el-form-item label="上级分类" prop="superiorId">
							<el-cascader :options="flcCategorySupplierTreeData"
								:props="{ checkStrictly: true, emitPath: false, value: 'id', label: 'categoryName' }"
								placeholder="请选择上级分类" clearable="" class="w100" v-model="ruleForm.superiorId">
				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="24" class="mb20">
						<el-form-item label="分类名" prop="categoryName">
							<el-input v-model="ruleForm.categoryName" placeholder="请输入分类名" maxlength="32"
...
import { addFlcCategorySupplier, updateFlcCategorySupplier, detailFlcCategorySupplier } from "/@/api/main/flcCategorySupplier";
import { getFlcCategorySupplierTree } from '/@/api/main/flcCategorySupplier';
...
const openDialog = async (row: any) => {
	...
	if (rowData.id)
		ruleForm.value = (await detailFlcCategorySupplier(rowData.id)).data.result;
	else
	...
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
				await addFlcCategorySupplier(values);
			} else {
				await updateFlcCategorySupplier(values);
			}
			closeDialog();
		} else {
			ElMessage({
