<template>
	<div class="flcCategory-container">
		<el-dialog v-model="isShowDialog" :width="400" style="height: 220px;" draggable="">
			<template #header>
				<div style="color: #fff">
				...
				<el-row :gutter="35">
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="24" class="mb20">
						<el-form-item label="分类名" prop="categoryName">
							<el-input v-model="ruleForm.categoryName" placeholder="请输入分类名" maxlength="32" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addFlcCategory, updateFlcCategory, detailFlcCategory } from "/@/api/main/flcCategory";
	import { getFlcCategoryTree } from '/@/api/main/flcCategory';
...
const rules = ref<FormRules>({
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailFlcCategory(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addFlcCategory(values);
				} else {
					await updateFlcCategory(values);
				}
				closeDialog();
			} else {
				ElMessage({
